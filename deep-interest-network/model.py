...
def behavior_attention(query, keys, behavior_input, padding_value=-2**32 + 1):
    ...
    # 時系列分repeat
    # Shape: (B, T, H)
    # B: バッチサイズ、T: 系列長、H: 次元数                                    # keys.get_shape() -> (None, 10, 16)
    query = tf.keras.backend.repeat_elements(query, keys.get_shape()[1], 1) # query (None, 1, 16) -> (None, 10, 16) keys(行動データのサイズに合わせるため繰り返す)

    # フィードフォワードネットワークへの入力
    attention_input = tf.concat([query, keys, query - keys, query * keys], # query (None, 10, 16), keys(None, 10, 16), query - keys (None, 10, 16), query * keys (None, 10, 16)
                                axis=-1)                                   # -> (None, 10, 64)

    # フィードフォワードネットワークでAttention weightを求める
    attention_weight = tf.keras.layers.Dense(
        80, activation='relu')(attention_input) # -> (None, 10, 80)
    attention_weight = tf.keras.layers.Dense(
        40, activation='relu')(attention_weight) # -> (None, 10, 40)
    attention_weight = tf.keras.layers.Dense(
        1, activation=None)(attention_weight) # -> (None, 10, 1)

    # Shape: (B, T, 1) => (B, 1, T)
    attention_weight = tf.transpose(attention_weight, (0, 2, 1)) # -> (None, 1, 10)

    # パディング用マスキング
    mask = tf.equal(behavior_input, 0)  # Shape: (B, T) (None, 10)
    mask = tf.expand_dims(mask, 1)  # Shape: (B, 1, T) (None, 1, 10)
    padding = tf.ones_like(attention_weight) * padding_value
    attention_weight = tf.where(mask, attention_weight, padding) # maskのところは重みをめっちゃ下げる

    # caled Dot-Product Attention的なやつ
    attention_weight = attention_weight / (keys.get_shape()[-1]**0.5) # TensorShape([None, 10, 16]) -> 16 -> 4  attention_weight (None, 1, 10)

    # softmax関数で正規化
    # 論文ではsoftmax関数に通していなかったが、実装では通していたのでそれに合わせる
    attention_weight = tf.nn.softmax(attention_weight) # ttention_weight (None, 1, 10)

    # Attention weightに従って過去の行動のEmbeddingのsum pooling
    attention_output = tf.matmul(attention_weight, keys)  # Shape: (B, 1, H)  (None, 1, 10) * (None, 10, 16) -> (None, 1, 16)
...
def deep_interest_network(features_info, seq_max_len=100, dropout_rate=0.5):
    ...
    inputs = [input for input in categorical_input_dict.values() # input -> user_id, product_id, product_category, buy_product, buy_product_category,
