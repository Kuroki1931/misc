...
const ApiContextProvider = (props) => {
  ...
  useEffect(() => {
    ...
    const getInbox = async () => {
      try {
        const res = await axios.get("http://localhost:8000/dm/inbox/", {
          headers: {
            Authorization: `Token ${token}`,
          },
        });
  ...
  const deleteProfile = async () => {
    try {
      await axios.delete(
        `http://localhost:8000/user/profile/${profile.id}/`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Token ${token}`,
  ...
  const sendDMCont = async (uploadDM) => {
    try {
      await axios.post(`http://localhost:8000/dm/message/`, uploadDM, {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Token ${token}`,
        },
  ...
  const changeApprovalRequest = async (uploadDataAsk, ask) => {
    try {
      const res = await axios.put(
        `http://localhost:8000/user/approval/${ask.id}/`,
        uploadDataAsk,
        {
          headers: {
            "Content-Type": "application/json",
      ...
      !resp[0]
        ...
        : await axios.put(
            `http://localhost:8000/user/approval/${resp[0].id}/`,
            newDataAskPut,
            {
              headers: {
                "Content-Type": "application/json",
